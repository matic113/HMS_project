...
{
    public static Prescription ConvertPrescriptionViewModelToPresciption(this Prescription prescription,
        PrescriptionViewModel ViewModel)
    {
        foreach (var (modelItem, vmItem) in prescription.PrescriptionItems.Zip(ViewModel.PrescriptionItems,
                     (modelItem, vmItem) => (modelItem, vmItem)))
    ...
    public static PrescriptionViewModel ConvertPresciptionToPrescriptionViewModel(this Prescription prescription)
    {
        ...
        PrescriptionVM.PrescriptionItems = prescription.PrescriptionItems
            .Select(i => i.ConvertPrescriptionItemToPrescriptionItemDoctorVM()).ToList();
    ...
    public static PrescriptionItemDoctorVM ConvertPrescriptionItemToPrescriptionItemDoctorVM(this PrescriptionItem item)
    {
        ...
        PrescriptionItemDoctorVM.Medications =
            item.Medications.Select(m => m.ConvertPrescriptionItemMedicationToPreItemMedVM()).ToList();

        return PrescriptionItemDoctorVM;
    }

    public static List<PrescriptionItemMedication> ConvertPreItemMedVMToPrescriptionItemMedication(
        this PrescriptionItem PreItem, List<PreItemMedVM> itemVM)
    {
