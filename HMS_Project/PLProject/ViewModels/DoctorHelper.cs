...
public static class DoctorHelper
{
    ...
    public static Doctor UpdatedDoctor(this Doctor doctor, DoctorViewModel doctorViewModel)
    {
        ...
        foreach (var (day, dayVM) in doctor.DoctorScheduleLookups.Zip(doctorViewModel.schedule,
        ...
        doctor.DoctorScheduleLookups.AddRange(
            doctorViewModel.schedule?.Skip(doctor.DoctorScheduleLookups.Count).Select(sv => sv.MapToEntity())
                .ToList() ?? new List<DoctorScheduleLookup>());
        return doctor;
    }
}