...
public class AppointmentPatientController : Controller
{
    ...
    public IActionResult Index(int? page, string filter)
    {
        ...
        var appointments = unitOfWork.Repository<Apointment>().Find(a => a.PatientUserId == UserId)
            .Include(a => a.Patient).Include(a => a.Doctor).Include(a => a.Clinic).ToList();

        if (filter == "Past")
            appointments = appointments.Where(a => /*a.ApointmentDate < DateOnly.FromDateTime(DateTime.Now) ||*/
                a.ApointmentStatus == ApointmentStatusEnum.Completed).ToList();
    ...
    public IActionResult Create(ClinicAvailabilityViewModel model)
    {
        ...
        if (model.SelectedClinicId.HasValue)
        {
            ...
            var availableDays = unitOfWork.Repository<DoctorScheduleLookup>()
                .Find(ds => doctors.Select(d => d.UserId).Contains(ds.DoctorUserId)).GroupBy(ds => ds.Day);

            var daysInCurrentMonth = DateTime.DaysInMonth(model.SelectedYear, model.SelectedMonth);

            List<DayAvailability> availabilityList = new();
