...
public class ClinicController : Controller
{
    ...
    public IActionResult AddSpecialization(ClinicSpecializationLookup ClinicSpecialization)
    {
        ...
        {
            var alreadyExist = unitOfWork.Repository<ClinicSpecializationLookup>().GetALL()
                .Any(c => c.Specialization == ClinicSpecialization.Specialization);
    ...
    public IActionResult Create(ClinicViewModel clinicViewModel)
    {
        if (ModelState.IsValid) // server side validation
            try
            {
                ...
                var AddedDoctor = unitOfWork.Repository<Doctor>()
                    .Find(d => clinicViewModel.DoctorsAddedId.Contains(d.Id));
    ...
    public IActionResult Edit([FromRoute] int Id, ClinicViewModel viewModel)
    {
        ...
        var updatedClinic = unitOfWork.Repository<Clinic>().Get(viewModel.Id);
    ...
    public IActionResult RemoveNurse(int nurseId, int clinicId)
    {
        try
        {
            ...
            if (nurse == null) return Json(new { success = false, message = "Nurse not found." });
