...
public class AppointmentReceptionistController : Controller
{
    ...
    public AppointmentReceptionistController(IUnitOfWork unitOfWork, IWebHostEnvironment Env,
    ...
    public IActionResult Index(int? page)
    {
        var spec = new BaseSpecification<Apointment>(a => /*=>a.ApointmentDate==DateOnly.FromDateTime(DateTime.Now)&&*/
            a.ApointmentStatus == ApointmentStatusEnum.Scheduled);
    ...
    public IActionResult Edit([FromRoute] int Id, ReceptionAppiontmentViewModel ViewModel)
    {
        ...
        if (!ModelState.IsValid) return Edit(ViewModel.Id);
    ...
    public IActionResult UpdatePreviousAppointment()
    {
        try
        {
            var spec = new BaseSpecification<Apointment>(
                a => /*a.ApointmentDate < DateOnly.FromDateTime(DateTime.Now) &&*/
                    a.ApointmentStatus == ApointmentStatusEnum.Scheduled);
