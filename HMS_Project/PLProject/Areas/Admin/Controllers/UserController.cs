...
public class UserController : Controller
{
    ...
    public async Task<IActionResult> Index(string filterRole, int page = 1)
    {
        ...
        if (!string.IsNullOrEmpty(filterRole)) users = users.Where(user => user.Roles.Contains(filterRole)).ToList();
    ...
    public async Task<IActionResult> Add()
    {
        var roles = await _roleManager.Roles
            .Select(role => new RoleViewModel { RoleId = role.Id, RoleName = role.Name }).ToListAsync();
    ...
    public async Task<IActionResult> Add(AddUserViewModel model)
    {
        ...
        if (!result.Succeeded)
        {
            foreach (var error in result.Errors) ModelState.AddModelError("Roles ", error.Description);
            return View(model);
        }

        await _userManager.AddToRolesAsync(user,
            model.Roles.Where(role => role.IsSelected).Select(role => role.RoleName));
    ...
    public async Task<IActionResult> ManageRoles(UserRolesViewModel model)
    {
        ...
        List<string> rolesRemoved = new();
        List<string> rolesAdded = new();
        ...
        {
            foreach (var role in rolesRemoved) RemoveFromTable(role, user.Id);

            foreach (var role in rolesAdded) await AddToTable(role, user.Id);
    ...
    {
        if (string.IsNullOrEmpty(UserId) || string.IsNullOrEmpty(tableName)) return;
        var _user = await _userManager.FindByIdAsync(UserId);

        switch (tableName)
        {
            case "Doctor":
                var DoctorSpecializationId = _unitOfWork.Repository<DoctorSpecializationLookup>()
                    .Find(d => d.Specialization == "General Practitioner").FirstOrDefault();

                _user.Doctor = new Doctor
                {
                    UserId = UserId,
                    DoctorSpecialization = DoctorSpecializationId ?? new DoctorSpecializationLookup
